-- Apply booking_default_status from master_settings when creating online appointments.
-- Previously the status was hardcoded to 'pending'; now the RPC reads the master's
-- configured default (pending | confirmed) and uses it, falling back to 'pending'
-- when the master has no settings row. source='online_booking' was already set.
...
BEGIN
  ...
  -- Read booking_default_status from master_settings; fall back to 'pending'.
  SELECT
    CASE WHEN booking_default_status IN ('pending', 'confirmed')
         THEN booking_default_status
         ELSE 'pending'
    END
  ...
  v_booking_status := coalesce(v_booking_status, 'pending');
