...
is the *system of record* — the master configures their identity, services,
schedule, and preferences there. The public site is a *read-mostly consumer* of
...
lets a master configure that the public site does *not yet* consume. That gap is
...
> the *same* database. To avoid collisions, treat **dashboard-owned tables as
...
does *not* read `master_settings` at all (the generated type exists in
...
| `booking_buffer_minutes` | `master_settings` | Ignored — bookings can butt up | Reserve buffer *after* each booking in slot search |
...
`master.seene/docs/business/online-booking.md` → *Slot search*.
...
When a question is about *how a setting behaves*, the dashboard docs are
